                      ...
                      m5::I2C_Class* i2c_bus)
{
    ...
    // I2C bus is required
    if (i2c_bus == nullptr) {
        ESP_LOGI(TAG, "Error: I2C bus instance is required. Please pass HAL->getI2C()");
        return false;
    }
    
    _i2c_bus = i2c_bus;
    ESP_LOGI(TAG, "Using provided I2C bus instance");
...
{
    return _i2c_bus->readRegister8(i2c_addr, reg_addr, 100000);
}

void M5EchoBase::wire_write_byte(uint8_t i2c_addr, uint8_t reg_addr, uint8_t value)
{
    _i2c_bus->writeRegister8(i2c_addr, reg_addr, value, 100000);
}
